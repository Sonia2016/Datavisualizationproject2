...
var monthItr = [];
  ...
  top50Miscellaneous = highFrequency(topMiscellaneous,20);
  top50Organization =  highFrequency(topOrganization,30);

   var freq = {
    words: []
  };

  for(var p in top50Person){
    freq.words.push({
      text:p,
      size: top50Person[p]
    })
  ...
  for(var p in top50Miscellaneous){
    freq.words.push({
      text:p,
  ...
  for(var p in top50Organization){
    freq.words.push({
      text:p,
...
postMessage({type: "end", lt: Persons});
